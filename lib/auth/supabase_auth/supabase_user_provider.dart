...
class FruviaSupabaseUser extends BaseAuthUser {
  FruviaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> fruviaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FruviaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
